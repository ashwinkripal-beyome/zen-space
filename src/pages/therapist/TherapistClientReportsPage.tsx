...
function TherapistReportCard({ report, listIndex, isFeatured }: ReportCardProps) {
  return (
    <Card
      className={cn(
      ...
      style={isFeatured ? { boxShadow: '0 0 0 1.5px rgb(52 211 153 / 0.55)' } : undefined}
    >
      <CardHeader className="space-y-2 pb-2 sm:pb-3">
        ...
        {isFeatured ? (
          <CardDescription className="text-sm text-muted-foreground sm:text-[15px]">
            This client&apos;s most recent report. Click to open.
          </CardDescription>
        ) : null}
      </CardHeader>
...
export function TherapistClientReportsPage() {
  const { user } = useAuth()
  ...
  const staggerVisible = usePageStaggerVisible(!loading, `${clientId}-${forbidden}-${reports.length}`)
  ...
  const base = `/app/therapist/clients/${clientId}/reports`
  ...
  return (
    <div className="space-y-6">
      ...
      <div style={pageStaggerItemStyle(1, staggerVisible)}>
        ...
        <p className="mt-2 text-muted-foreground">Open a report to read the full Zen Plan, ritual, and assessment answers.</p>
      </div>
      ...
      {reports.length === 0 ? (
        ...
        >
          <CardContent className="flex flex-col items-center gap-4 py-14 text-center">
            ...
            <p className="text-muted-foreground">No reports yet for this client.</p>
            <Button asChild variant="zenOutline" size="sm">
              <Link to={`/app/therapist/clients/${clientId}/observations`}>Observations &amp; generate</Link>
            </Button>
          </CardContent>
        </Card>
      ) : (
        <>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
            <Link
              ...
              style={pageStaggerItemStyle(2, staggerVisible)}
            >
              <TherapistReportCard report={latest} listIndex={0} isFeatured />
            </Link>
          </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
                {previous.map((report, j) => (
                  <Link
                    ...
                    style={pageStaggerItemStyle(4 + j, staggerVisible)}
                  >
                    <TherapistReportCard report={report} listIndex={j + 1} isFeatured={false} />
                  </Link>
                ))}
              </div>
            </section>
